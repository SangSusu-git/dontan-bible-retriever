...
def clean_text(text: str) -> str:
    """크롤러 아티팩트를 제거해 공식(대한성서공회) 개역한글 표기에 맞춘다.

    2026-08 공식 온라인 개역한글(bskorea.or.kr)과의 대조로 확정한 규칙:
      제거(공식본에 없음이 확인된 크롤러 오염):
      - 작은따옴표 인용부호 '...'   (4,071구절)
      - 물음표 앞 공백 "느냐 ?" → "느냐?"  (1,366구절)
      - 나열 쉼표 "셈과, 함과," → "셈과 함과"  (875구절)
      - 한자 괄호 주석 "신(神)" → "신"  (76구절)
      보존(공식본에도 있는 표기):
      - 시편 표제 "(다윗의 시)", 삽입구 괄호 구절, 합절 "(18절과 같음)",
        단어에 붙은 느낌표("할렐루야!")
    """
    text = html.unescape(text)  # &#x27; -> '
    text = text.replace("`", "'")  # 인용 여는 부호 정규화(아래에서 일괄 제거)
    # 크롤러 아티팩트인 " !"(공백+느낌표)만 제거. 단어에 바로 붙은 진짜 느낌표
    # (예: "할렐루야!", "두려워 말라!")는 원문 표현이므로 보존한다.
    text = re.sub(r"\s+!", "", text)
    text = text.replace("'", "")                 # 인용부호 제거
    text = re.sub(r"\s+\?", "?", text)           # "느냐 ?" -> "느냐?"
    text = re.sub(r"([과와]),", r"\1", text)     # "셈과," -> "셈과"
    text = re.sub(r"\([一-鿿]+\)", "", text)  # 한자 괄호 주석 제거
